...
describe('Content Routes', () => {
  ...
  describe('GET /courses/:courseId/modules/:moduleId/sections/:sectionId/content', () => {
    it('should return markdown content for a section with section read access', async () => {
      const token = await createToken('ADMIN')

      const response = await request(app)
        .get(
          `/courses/${testCourseId}/modules/${testModuleId}/sections/${testSectionId}/content`
        )
    ...
    it('should reject a non-existent section at the section read policy', async () => {
      const token = await createToken('ADMIN')
      const fakeSectionId = '00000000-0000-0000-0000-000000000000'

      await request(app)
        .get(
          `/courses/${testCourseId}/modules/${testModuleId}/sections/${fakeSectionId}/content`
        )
        .set('Authorization', token)
        .expect(403)
    })
  ...
  describe('PATCH /courses/:courseId/modules/:moduleId/sections/:sectionId/content', () => {
    it('should update markdown content with section write access', async () => {
      const token = await createToken('ADMIN')
    ...
    it('should reject a non-existent section at the section write policy', async () => {
      const token = await createToken('ADMIN')
      const fakeSectionId = '00000000-0000-0000-0000-000000000000'
