...
import {
  findAllSections,
  findSectionById, findSectionByIdOnly,
  createSection,
  updateSection,
  deleteSection,
} from '../models/section';
...
export async function updateSectionController(
  ...
  try {
    ...
    const section = await updateSection(params.sectionId, parsedBody, moduleIdParams.moduleId);
    res.json(section);
  } catch (error) {
...
/**
 * Get a section by its ID only (used by the search results enrichment).
 */
export async function getSectionByIdOnlyController(
  req: Request,
  res: Response
): Promise<void> {
  try {
    ...
    const params = paramsSchema.parse(req.params);
    const section = await findSectionByIdOnly(params.sectionId);
    if (!section) {
      res.status(404).json({ error: 'Section not found' });
      return;
    }
    ...
    console.error('Validation error in getSectionByIdOnlyController:', error);
    if (isZodError(error)) {
      res.status(400).json({ error: zodErrorDetails(error) });
    } else {
      console.error(error);
...
export async function deleteSectionController(
  ...
  try {
    ...
    const params = paramsSchema.parse(req.params);
    await deleteSection(params.sectionId, moduleIdParams.moduleId);
    res.status(204).send();
  } catch (error) {
